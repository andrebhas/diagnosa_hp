<?php

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Pertanyaan_model extends CI_Model
{

    public $table = 'pertanyaan';
    public $id = 'id';
    public $order = 'DESC';

    function __construct()
    {
        parent::__construct();
    }

    // get all
    function get_all()
    {
        $this->db->order_by($this->id, $this->order);
        return $this->db->get($this->table)->result();
    }

    // get data by id
    function get_by_id_kerusakan($id,$limit="")
    {
        $this->db->where('id_jenis_kerusakan', $id);
        $this->db->order_by('kode', 'ASC');
        if(!empty($limit)){
            $this->db->limit($limit);    
        }
        return $this->db->get($this->table)->result();
    }

    function get_by_kode($kode)
    {
        $this->db->where('kode', $kode);
        return $this->db->get($this->table)->row();
    }

    // get data by id
    function get_by_id($id)
    {
        $this->db->where($this->id, $id);
        return $this->db->get($this->table)->row();
    }

    function get_by_kerusakan($id_jenis_kerusakan)
    {
        $this->db->where('id_jenis_kerusakan', $id_jenis_kerusakan);
        return $this->db->get($this->table)->result();
    }
    
    // get total rows
    function total_rows($q = NULL) {
        $this->db->like('id', $q);
	$this->db->or_like('pertanyaan', $q);
	$this->db->or_like('kode', $q);
	$this->db->or_like('id_jenis_kerusakan', $q);
	$this->db->or_like('ya', $q);
	$this->db->or_like('tidak', $q);
	$this->db->from($this->table);
        return $this->db->count_all_results();
    }

    // get data with limit and search
    function get_limit_data($limit, $start = 0, $q = NULL) {
        $this->db->order_by($this->id, $this->order);
        $this->db->like('id', $q);
        $this->db->or_like('pertanyaan', $q);
        $this->db->or_like('kode', $q);
        $this->db->or_like('id_jenis_kerusakan', $q);
        $this->db->or_like('ya', $q);
        $this->db->or_like('tidak', $q);
        $this->db->limit($limit, $start);
        return $this->db->get($this->table)->result();
    }

    // insert data
    function insert($data)
    {
        $this->db->insert($this->table, $data);
    }

    // update data
    function update($id, $data)
    {
        $this->db->where($this->id, $id);
        $this->db->update($this->table, $data);
    }

    // delete data
    function delete($id)
    {
        $this->db->where($this->id, $id);
        $this->db->delete($this->table);
    }

}

/* End of file Pertanyaan_model.php */
/* Location: ./application/models/Pertanyaan_model.php */
/* Please DO NOT modify this information : */
/* This file generated by Andre Bhaskoro (+62 82 333 817 317) At : 2017-05-30 15:30:36 */
/* http://bhas.web.id */